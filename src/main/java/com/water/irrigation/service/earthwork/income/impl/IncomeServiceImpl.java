package com.water.irrigation.service.earthwork.income.impl;
...
public class IncomeServiceImpl implements IncomeService {
     ...
     */
    @Override
    public Page<Income> findAll(IncomeDto conditions, Pageable pageable) {
        return incomeDao.findAll(getSpecification(conditions), pageable);
    }
     ...
     */
    @Override
    public Income findOne(Long indocno) {
        return incomeDao.findById(indocno).get();
    }

    /**
     * 添加资产
     * @param income 要添加的资产信息
     * @return 添加的资产信息
     */
    @Override
    ...
    public Income update(Income income) {
        Income orgObj =
                this.incomeDao.findById(income.getIndocno()).get();
        if(orgObj!=null) {
     ...
     */
    @Override
    public void delete(List<Income> sysCompanys) {
        for(Income income : sysCompanys ) {
            incomeDao.deleteById(income.getIndocno());
        }
    }
     ...
     * @return 查询条件
     */
    private Specification<Income> getSpecification(IncomeDto condition){
        // 查询条件构造
        Specification<Income> spec = new Specification<Income>(){

            private static final long serialVersionUID = 3652606507255419478L;

            @Override
            public Predicate toPredicate(Root<Income> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
